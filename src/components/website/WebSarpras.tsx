...
export default function WebSarpras() {
  const [webContent, setWebContent] = useState<WebSectionContent>(() => {
    return INITIAL_WEB_CONTENT.find(c => c.id === 'sarpras')!;
  });
  ...
  useEffect(() => {
    const unsubscribe = syncCollection<WebSectionContent>(
      'web_content',
      (data) => {
        const found = data.find(c => c.id === 'sarpras');
        if (found) {
          setWebContent(found);
        }
        setLoading(false);
  ...
  const facilities = webContent.facilities || [];
  ...
  return (
    <div className="space-y-10 pb-12 font-sans text-left">
      ...
      <div 
        ...
        style={{ 
          backgroundImage: `linear-gradient(135deg, rgba(13, 92, 58, 0.9) 0%, rgba(4, 47, 26, 0.8) 100%), url('${webContent.headerBgImage || 'https://lh3.googleusercontent.com/d/18Ky3AJ-jAzh49hAhH6_R_K24aSUp4OTz'}')` 
        }}
      >
        <div className="relative z-10 space-y-2">
          <span className="text-[10px] bg-amber-400 text-slate-950 font-bold px-2.5 py-0.5 rounded-full uppercase tracking-wider">Infrastruktur Unggul</span>
          <h1 className="text-2xl md:text-3xl lg:text-4xl font-black tracking-tight">{webContent.headerTitle || 'Bidang Sarana & Prasarana'}</h1>
          <p className="text-xs md:text-sm text-slate-200">{webContent.headerSubtitle || 'Mewujudkan Infrastruktur Sekolah yang Modern, Nyaman, Bersih, Sehat, dan Berwawasan Lingkungan.'}</p>
        </div>
      </div>

      {/* TWO COLUMN: WAKA SARPRAS PROFILE & STAFF / PROGRAM */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        
        {/* SIDEBAR: HEAD OF SARPRAS */}
            ...
            <div className="bg-white border border-slate-200/80 rounded-3xl p-6 text-center shadow-md relative overflow-hidden group">
              <div 
                onClick={() => setActiveStaff({
                  ...
                  department: webContent.headSubject || 'Sarana Prasarana',
                  hobby: webContent.headHobby || '-',
                  motto: webContent.headMotto || 'Dedikasi tinggi untuk sekolah.'
                })}
                className="relative w-32 h-40 mx-auto mb-4 p-1 bg-gradient-to-tr from-amber-400 to-emerald-700 rounded-xl shadow-md cursor-pointer hover:scale-[1.03] transition-transform duration-300 group/waka"
              >
                ...
                <div className="absolute -bottom-1.5 -right-1.5 bg-emerald-700 text-white rounded-lg p-1.5 shadow-md">
                ...
                <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-3.5 hidden group-hover/waka:flex flex-col bg-slate-950/95 text-white rounded-2xl p-4 shadow-2xl border border-white/20 w-56 text-left z-30 transition-all duration-300 pointer-events-none">
                  <div className="space-y-2 text-[10px]">
                    ...
                    <div>
                      ...
                      <span className="font-semibold text-slate-100">{webContent.headSubject || 'Sarana Prasarana'}</span>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <span className="italic text-slate-200">"{webContent.headMotto || 'Dedikasi tinggi untuk sekolah.'}"</span>
                    </div>
                  </div>
                  {/* Tooltip Arrow */}
              ...
              <h3 className="font-extrabold text-emerald-900 text-sm leading-tight mb-2">{webContent.headName}</h3>
              ...
              {staff.map((s) => (
                <button
                  ...
                  onClick={() => setActiveStaff({
                    ...s,
                    born: s.born || '-',
                    department: s.department || s.subject || '-',
                    hobby: s.hobby || '-',
                    motto: s.motto || 'Dedikasi tinggi.'
                  })}
                  className="w-full flex items-center gap-3.5 p-3 bg-white border border-slate-200/80 rounded-2xl hover:border-emerald-700 transition-all text-left shadow-xs hover:translate-x-1 cursor-pointer group/staff relative"
                >
                  <div className="w-11 h-11 rounded-lg border-2 border-emerald-700 overflow-hidden shrink-0 bg-slate-200">
                    <img 
                      src={s.image} 
                      alt={s.name}
                      className="w-full h-full object-cover object-top"
                  ...
                  <div>
                    <h4 className="text-xs font-black text-slate-800 group-hover/staff:text-emerald-900 transition-colors leading-tight">
                      {s.name}
                    </h4>
                  ...
                  <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-3.5 hidden group-hover/staff:flex flex-col bg-slate-950/95 text-white rounded-2xl p-4 shadow-2xl border border-white/20 w-56 text-left z-30 transition-all duration-300">
                    <div className="space-y-2 text-[10px]">
                      ...
                      <div>
                        <span className="text-slate-400 block font-bold">Bidang:</span>
                        <span className="font-semibold text-slate-100">{s.department || s.subject || '-'}</span>
                      </div>
                      <div>
                        <span className="text-slate-400 block font-bold">Hobi:</span>
        ...
        <section className="lg:col-span-8 space-y-8">
          ...
          <div className="space-y-4">
            <h2 className="text-lg font-black text-slate-800 flex items-center gap-2 border-b border-slate-100 pb-2">
              ...
              <span>Program Kerja Unggulan Sarpras</span>
            </h2>
            ...
            <div className="grid grid-cols-1 gap-4">
              {webContent.programs.map((prog, idx) => (
                <div 
                  key={idx}
                  className="bg-white border border-slate-200/80 rounded-2xl p-5 flex items-start gap-5 hover:border-emerald-500/30 transition-all hover:bg-emerald-50/10 hover:shadow-lg hover:scale-[1.01] duration-300"
                >
                  <div className="w-12 h-12 rounded-xl bg-emerald-50 text-emerald-800 flex items-center justify-center shrink-0">
                    <WebIcon name={prog.icon || '🏆'} className="w-6 h-6 text-emerald-800" />
                  </div>
          ...
          <div className="bg-white border border-slate-200/80 rounded-3xl p-5 shadow-sm space-y-4">
            <h3 className="text-xs font-bold text-emerald-800 uppercase tracking-widest flex items-center gap-1.5">
              ...
              <span>Dokumentasi Sarana &amp; Prasarana Unggulan</span>
            </h3>
            ...
            <div className="relative rounded-2xl overflow-hidden h-[260px] border border-slate-100 group">
              <div 
                ...
                style={{ 
                  backgroundImage: `linear-gradient(to top, rgba(13, 92, 58, 0.95) 0%, rgba(0,0,0,0.2) 60%), url(${slides[currentSlide].image})`
                }}
              />
      ...
      <div className="bg-white border border-slate-200/80 rounded-3xl p-6 md:p-8 shadow-xs space-y-6">
        <h2 className="text-lg font-black text-slate-800 flex items-center gap-2 border-b border-slate-100 pb-3">
          ...
          <span>Kondisi &amp; Profil 10 Sarana Utama Sekolah</span>
        </h2>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {facilities.map((fac) => (
            <button
              ...
              className="bg-slate-50 hover:bg-white border border-slate-200/60 hover:border-emerald-700 p-5 rounded-2xl text-center flex flex-col items-center justify-center gap-3 transition-all cursor-pointer hover:-translate-y-1.5 hover:shadow-md group min-h-[140px]"
            >
              <div className="w-11 h-11 rounded-full bg-white group-hover:bg-emerald-900 border border-slate-100 group-hover:border-emerald-950 shadow-inner flex items-center justify-center text-emerald-800 group-hover:text-white transition-all scale-[1.03]">
                <WebIcon name={fac.icon} className="w-5 h-5" />
              </div>
              <span className="text-xs font-black text-slate-800 group-hover:text-emerald-900 transition-colors leading-snug">
                {fac.name}
              </span>
            </button>
          ))}
        ...
        {activeStaff && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="bg-gradient-to-b from-slate-900 to-emerald-950 text-white rounded-3xl w-full max-w-md p-6 shadow-2xl relative z-10 border border-white/10 text-left"
            >
              <button
                onClick={() => setActiveStaff(null)}
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3.5 text-xs">
                  ...
                  <div className="popup-row flex items-start gap-2.5">
                    ...
                    <span><strong>Bidang:</strong> {activeStaff.department}</span>
                  </div>
                  <div className="popup-row flex items-start gap-2.5">
        ...
        {activeFacility && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              className="bg-gradient-to-b from-slate-900 to-emerald-950 text-white rounded-3xl w-full max-w-lg p-6 shadow-2xl relative z-10 border border-white/10 max-h-[90vh] overflow-y-auto scrollbar-thin"
            >
              <button
                onClick={() => setActiveFacility(null)}
                className="absolute right-5 top-5 text-white/75 hover:text-white bg-white/10 hover:bg-white/20 p-1.5 rounded-full transition-colors cursor-pointer"
              ...
              <div className="space-y-5">
                <div className="flex items-center gap-3 text-left">
                  <div className="w-11 h-11 rounded-2xl bg-white/10 flex items-center justify-center text-amber-400 shrink-0 border border-white/10">
                    <WebIcon name={activeFacility.icon} className="w-5 h-5" />
                  </div>
                  <div className="space-y-0.5">
                    ...
                    <h3 className="text-lg font-black leading-tight">{activeFacility.name}</h3>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 gap-2.5 text-xs text-left bg-white/5 p-4 rounded-2xl border border-white/10">
                  <div className="flex items-center gap-2">
                    ...
                    <span><strong>Pengelola:</strong> {activeFacility.coordinator}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <CheckCircle className="w-4 h-4 text-amber-300" />
                    <span><strong>Kondisi:</strong> {activeFacility.condition}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Users className="w-4 h-4 text-amber-300" />
                    <span><strong>Kapasitas:</strong> {activeFacility.capacity}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Star className="w-4 h-4 text-amber-300" />
                    <span><strong>Fitur Utama:</strong> {activeFacility.mainFeatures}</span>
                  </div>
                </div>
                ...
                <div className="space-y-2 text-left">
                  <h4 className="text-xs font-bold text-amber-300 uppercase tracking-wider flex items-center gap-1">
                    ...
                    <span>Daftar Inventaris Utama</span>
                  </h4>
                  ...
                  <div className="border border-white/10 rounded-xl overflow-hidden text-xs">
                    <table className="w-full text-left">
                      <thead className="bg-white/10 font-bold border-b border-white/10">
                        <tr>
                          <th className="p-2.5">Item Inventaris</th>
                          <th className="p-2.5">Detail Kondisi</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-white/5">
                        {activeFacility.inventory.map((inv, idx) => (
                          <tr key={idx}>
                            <td className="p-2.5 text-slate-200 font-bold">{inv.item}</td>
                            <td className="p-2.5 text-slate-300">{inv.detail}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                {(() => {
                  const facilityImages = (activeFacility.images && activeFacility.images.length > 0)
                    ? activeFacility.images
                    : popupImages;
                  const validIndex = facilityImages.length > 0 ? (currentPopupSlide % facilityImages.length) : 0;

                  return (
                    <div className="space-y-2 text-left">
                      <h4 className="text-xs font-bold text-amber-300 uppercase tracking-wider flex items-center gap-1">
                        <Camera className="w-4 h-4" />
                        <span>Galeri Dokumentasi {activeFacility.name} ({facilityImages.length} Foto)</span>
                      </h4>

                      <div className="relative rounded-xl overflow-hidden h-[160px] border border-white/10">
                        <img 
                          src={facilityImages[validIndex] || facilityImages[0]} 
                          alt={`Dokumentasi ${activeFacility.name}`}
                        ...
                        {facilityImages.length > 1 && (
                          <>
                            <button 
                              onClick={(e) => {
                                e.stopPropagation();
                                setCurrentPopupSlide((prev) => (prev - 1 + facilityImages.length) % facilityImages.length);
                              }}
                            ...
                            <button 
                              onClick={(e) => {
                                e.stopPropagation();
                                setCurrentPopupSlide((prev) => (prev + 1) % facilityImages.length);
                              }}
                            ...
                            <div className="absolute bottom-2 right-2 bg-slate-950/85 px-2 py-0.5 rounded-md text-[9px] font-bold text-slate-300 font-mono">
                              {validIndex + 1} / {facilityImages.length}
                            </div>
                          </>
                        )}
                      </div>
