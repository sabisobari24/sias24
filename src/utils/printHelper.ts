...
export const printHTML = (htmlContent: string) => {
  ...
  pdfBtn.className = 'px-3.5 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-xl text-xs font-bold transition-all shadow-sm flex items-center gap-1.5 cursor-pointer';
  pdfBtn.innerHTML = `
    <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
  ...
  pdfBtn.onclick = async () => {
    ...
    try {
      ...
      if (!targetDoc || !targetDoc.body) {
        alert('Dokumen belum siap. Silakan coba sesaat lagi.');
        return;
      }
      ...
      let fileName = 'laporan_resmi_smpn50.pdf';
      const titleMatch = htmlContent.match(/<title>([\s\S]*?)<\/title>/i);
      if (titleMatch && titleMatch[1]) {
        fileName = `${titleMatch[1].trim().replace(/[^a-zA-Z0-9]/g, '_')}.pdf`;
      }
      pdf.save(fileName);
    } catch (err) {
      console.error('Failed to generate PDF from printHTML:', err);
      alert('Gagal menghasilkan PDF secara otomatis. Gunakan tombol "Cetak Langsung" lalu pilih "Save as PDF".');
    } finally {
